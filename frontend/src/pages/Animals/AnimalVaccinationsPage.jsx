              ...
              <div className="relative" ref={filterRef}>
                <button onClick={() => setIsFilterOpen(p => !p)}
                  className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-bold border transition-all ${
                    filterStatus !== 'all' ? 'bg-white text-[#1b4d2c] border-white' : 'bg-white/10 border-white/20 text-white hover:bg-white/20'
                  }`}>
        ...
        {loading.vaccinations ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {vaccinations.filter(vac => {
              ...
              return (
                  <div key={vac._id}
                    className={`bg-[#f2f8f3] border border-[#154b23]/10 rounded-[24px] p-5 shadow-sm hover:shadow-md transition-all duration-300 relative flex flex-col group ${!vac.is_active ? 'opacity-70' : ''}`}>
                    <div className="flex justify-between items-start gap-2 mb-4">
                      <span className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-bold border shadow-sm ${status.color}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${status.dot}`}></span>
                        {status.label}
                      </span>
                      <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-bold ${vac.vaccine_type === 'one_time' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'bg-[#1b4d2c]/10 text-[#1b4d2c] border border-[#1b4d2c]/20'}`}>
                        {vac.vaccine_type === 'one_time' ? 'لمرة واحدة' : 'متكرر'}
                      </span>
                    </div>

                    <div className="cursor-pointer flex-1" onClick={() => handleOpenDetails(vac)}>
                      <h3 className="font-extrabold text-[#1b4d2c] text-lg leading-tight group-hover:text-[#154b23] transition-colors mb-2">
                        {vac.vaccine_name}
                      </h3>
                      <div className="space-y-1.5 text-xs text-gray-500 font-medium">
                        {vac.dose_ml && (
                          <p className="flex items-center gap-1"><span className="text-gray-400">حجم الجرعة:</span><strong className="font-bold text-gray-700">{vac.dose_ml} مل</strong></p>
                        )}
                        {vac.vaccine_type === 'recurring' && vac.repeat_every_months && (
                          <p className="flex items-center gap-1"><span className="text-gray-400">يتكرر كل:</span><strong className="font-bold text-gray-700">{vac.repeat_every_months} شهر</strong></p>
                        )}
                        {vac.vaccine_type === 'recurring' ? (
                          <p className="flex items-center gap-1 text-gray-600">
                            <span className="text-gray-400">الجرعة القادمة:</span>
                            <span className="font-extrabold text-gray-900">{formatDate(vac.next_due_date)}</span>
                          </p>
                        ) : (
                          <p className="flex items-center gap-1 text-gray-600">
                            <span className="text-gray-400">موعد اللقاح:</span>
                            <span className="font-extrabold text-gray-900">{formatDate(vac.scheduled_date)}</span>
                          </p>
                        )}
                      </div>
                    </div>

                    <div className="mt-5 pt-4 border-t border-[#154b23]/10 flex items-center justify-between gap-2">
                      <button onClick={() => handleOpenDetails(vac)}
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-white hover:bg-gray-50 text-[#1b4d2c] rounded-xl text-sm font-bold transition-all border border-[#154b23]/10">
                      ...
                      <div className="flex items-center gap-1.5">
                        {vac.is_active && !vac.completed && (
                          <>
                            <button onClick={() => handleConfirmCompleted(vac)}
                              className="flex items-center justify-center gap-1 px-3 py-2 bg-[#1b4d2c] hover:bg-[#154b23] text-white rounded-xl text-sm font-bold transition-all shadow-sm">
                              <Check className="w-3.5 h-3.5" />تأكيد
                            </button>
                            <button onClick={() => handleOpenPostpone(vac)}
                              className="p-2 text-blue-600 hover:bg-white rounded-lg border border-transparent hover:border-[#154b23]/10 transition-all" title="تأجيل الموعد">
                        ...
                        {vac.is_active && vac.vaccine_type === 'recurring' && (
                          <button onClick={() => handleStop(vac._id)}
                            className="p-2 text-amber-600 hover:bg-white rounded-lg border border-transparent hover:border-[#154b23]/10 transition-all" title="إيقاف متابعة اللقاح">
                            <PauseCircle className="w-4 h-4" />
                          </button>
                        )}
                        <button onClick={() => handleDelete(vac._id)}
                          className="p-2 text-rose-600 hover:bg-white rounded-lg border border-transparent hover:border-[#154b23]/10 transition-all" title="حذف السجل">
