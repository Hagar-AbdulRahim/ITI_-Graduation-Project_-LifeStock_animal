    ...
    <>
      ...
      >
        ...
        <nav className='flex-1 overflow-y-auto py-4 px-3 space-y-2'>
          {SIDEBAR_LINKS.map((link) => {
            const path = link.id === 'dashboard' ? `/farms/${farmId}` : `/farms/${farmId}${link.path}`;
            return (
              <NavLink
                key={link.id}
                to={path}
      ...
      {showEmergencyConfirm && (
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-stone-100 p-6 animate-in fade-in zoom-in-95 duration-200' dir="rtl">
            <div className='flex items-center gap-2 mb-4 text-red-600 font-bold'>
              <span className='text-xl'>🚨</span>
              <h3 className='text-sm font-bold text-stone-800'>تأكيد طلب دعم الطوارئ</h3>
            </div>
            
            <p className='text-xs text-stone-500 mb-6 leading-relaxed'>
              أنت على وشك تفعيل دعم الطوارئ. سيقوم هذا الإجراء بإرسال تنبيه عاجل لجميع الأطباء البيطريين المناوبين والمسؤولين عن رعاية الحظيرة وتوفير تفاصيل الاتصال الفورية. هل أنت متأكد من الاستمرار؟
            </p>

            <div className='flex gap-3 justify-end text-xs'>
              <button 
                onClick={() => {
                  toast.success('تم إرسال بلاغ الطوارئ بنجاح! سيتم التواصل معك فورا.');
