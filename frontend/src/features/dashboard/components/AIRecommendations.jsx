...
const priorityStyle = {
  ...
  medium: { label: 'أولوية متوسطة',   bg: 'bg-[#fef3c7]', text: 'text-[#b45309]' },
...
function RecommendationCard({ rec, index }) {
  ...
  return (
    <div
      ...
      className='p-4 rounded-xl border border-stone-100 bg-stone-50 hover:bg-white hover:shadow-sm transition-all duration-300'
    >
      <div className='flex items-start justify-between gap-2 mb-2'>
        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${p.bg} ${p.text}`}>
          {p.label}
        </span>
      </div>
...
export default function AIRecommendations() {
  const mockRecs    = useSelector((state) => state.dashboard.aiRecommendations);
  const farmStats   = useSelector((state) => state.farm.farmStats);
  const currentFarm = useSelector((state) => state.farm.currentFarm);

  const recs = farmStats?.stats?.ai_recommendations ?? mockRecs;
    ...
    <div className='bg-white rounded-2xl p-5 shadow-sm border border-stone-100 h-full flex flex-col'>
      ...
      <div className='flex items-center justify-between mb-4'>
        <div className='flex items-center gap-2'>
          ...
          <div>
            <p className='text-sm font-bold text-stone-700'>توصيات الذكاء الاصطناعي</p>
            <p className='text-[11px] text-stone-400'>
              رؤى مخصصة لمزرعة {currentFarm?.name || '...'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className='flex-1 space-y-3 overflow-y-auto'>
        {recs.length === 0 ? (
          <div className='flex flex-col items-center justify-center h-full py-8 text-stone-400 text-sm text-center'>
            ...
            <p className='text-xs mt-1'>أضف حيوانات للحصول على تحليلات الذكاء الاصطناعي</p>
          </div>
        ) : (
          recs.map((rec, i) => (
